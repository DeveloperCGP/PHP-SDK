...
class H2HVoidAddonPaymentsSDKTest extends TestCase
{
    ...
    public function testReciveResponse()
    {
        ...
        $createVoidRequestMock = $this->getMockBuilder(CreateVoidRequest::class)
            ->setConstructorArgs([$ivGeneratorMock])
            ->onlyMethods(['requestCurl']) // Mock only sendRequest method
            ->getMock();
        $createVoidRequestMock->method('requestCurl')->willReturn(['response' => $reponseMock, 'status_code' => 200, 'message' => null]);
        $addonPaymentsSDK = new AddonPaymentsSDK($config);
        $reflectionClass = new ReflectionClass($addonPaymentsSDK);
        $property = $reflectionClass->getProperty('createVoidRequest'); // Adjust the property name if it's different
        $property->setAccessible(true); // Make the property accessible
        $property->setValue($addonPaymentsSDK, $createVoidRequestMock); // Inject the mock
        $sdk = $addonPaymentsSDK;
        $sendRequest = $sdk->sendVoidPaymentRequest();
        ...
        $excpectedEncryptedRequest = 'g4lqI0iz6/VzgHQjJK3Wxb0eOI+6REw11PvpTroAGp1i8Rnogzlx+XNb3mS8gmE8fo9jVLo0XIlCMgX20TvAAiTRD94j4fJUJAiEGVhrp6U3AaYyqoJ5GpO7737iPIUoeBHmEKZWL3BCdFZdLH/ZTwvUhp5Jes2rATwrdkr6+h3rrIXOw9TRFGhDi7DuxhpF6kN/tlEGWZaJenRhw3BOsYT4TnI0bYBdyb4eoOq/CO0=';
        $expectedFormatedRequest = 'paymentSolution=caixapucpuce&merchantTransactionId=87145&transactionId=7693681&productId=111166625&merchantParams=sdk%3Aphp%3Bversion%3A1.0.2%3Btype%3AVoid&merchantId=1111111';
        ...
        $this->assertContains('sdk:php;version:1.0.2;type:Void', $merchantParams, 'Assert merchantParams in request');
    ...
    public function testExceptionForMissingParameters($missingParameter, $expectedExceptionMessage)
    {
        ...
        $addonPaymentsSDK->sendVoidPaymentRequest();
    }
