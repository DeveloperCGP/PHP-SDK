...
class CreateCaptureRequest
{
    ...
    /**
     * Initializes a capture payment request with necessary parameters.
     *
     ...
     */
    public function initCapturePaymentRequest(int $merchantId, string $merchantPassword, array $otherConfigurations, bool|null $production, string|null $baseUrl): void
    {
        ...
        if (count($otherConfigurations) > 0) {
            if (isset($otherConfigurations['merchantParams']) && !empty($otherConfigurations['merchantParams'])) {
                $otherConfigurations['merchantParams'] .= ';sdk:php;version:'. $this->packageVersion . ';type:Capture';
            } else {
                $otherConfigurations['merchantParams'] = 'sdk:php;version:'. $this->packageVersion . ';type:Capture';
            }
        }
    ...
    /**
     * Encrypts the capture request data for secure transmission.
     *
    ...
    /**
     * Sends the capture request to the server.
     *
    ...
    public function sendRequest(): array
    {
        ...
        if (isset($this->baseUrl)) {
            ...
            if ($this->production) {
                $url = RequestsPaths::CAPTURE_PROD;
            } else {
                $url = RequestsPaths::CAPTURE_STG;
            }
        }
        $data_url = [
        ...
        if (is_string($responseData)) {
            $this->logMessage("Request processed successfully.", $responseData, $headers, $data_url, "capture", "creditcards", $this->otherConfigurations['merchantTransactionId'], $this->otherConfigurations);
            ...
            // Payment failed
            $this->logError("No response data received or cURL request failed.");
            return [
                'result' => 'Fail',
                'response' => 'ERROR',
            ];
    ...
    /**
     * Retrieves the response of the Capture request.
     *
